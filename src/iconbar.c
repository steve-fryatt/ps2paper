...
void iconbar_initialise(void)
{
	...
	icons_printf(iconbar_info_window, ICON_PROGINFO_AUTHOR, "\xa9 Stephen Fryatt, 2013-%s", date + 7);
...
static void iconbar_menu_selection(wimp_w w, wimp_menu *menu, wimp_selection *selection)
{
	...
	case ICONBAR_MENU_HELP:
		os_cli("%Filer_Run <PS2Paper$Dir>.!Help");
		break;
...
static osbool iconbar_proginfo_web_click(wimp_pointer *pointer)
{
	char		temp_buf[URL_LENGTH];

	msgs_lookup("SupportURL:http://www.stevefryatt.org.uk/software/", temp_buf, URL_LENGTH);
	url_launch(temp_buf);
